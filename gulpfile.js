...
var minifyCSS = require('gulp-minify-css'); //css压缩操作
var uglify = require('gulp-uglify'); //丑化编码
        ...
        .pipe(webserver({
            ...
            middleware: function(req, res, next) {
                    ...
                    switch (urlObj.pathname) {
                        ...
                        case '/api/user':
                            res.setHeader('Content-Type', 'application/json');
                            fs.readFile('mock/user.json', 'utf-8', function(err, data) {
                                res.end(data);
                            });
                            return;
                        case '/api/like':
...
gulp.task('sass', function() {
    return gulp.src('./src/styles/**/*.*').pipe(sass()).pipe(gulp.dest('www/css'));
})
...
gulp.task('packjs', function() {
    return gulp.src(jsFiles)
        ...
        .pipe(webpack()) //打包
        // .pipe(uglify())  //丑化  安全方面
        .pipe(gulp.dest('www/js')) //放置于www下的js文件内
